...
import { X, Mail, Lock, User, ArrowRight, LogIn, CheckCircle2, Shield, RefreshCw } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  // Handle Official Google Identity Services Success
  const handleGoogleSuccess = async (credentialResponse: any) => {
    ...
    try {
      ...
      setIsLoading(false);
      setErrorMessage(e.message || 'Google Sign-In verification failed.');
    }
  };

  // Google OAuth Authorization Redirect / Fallback
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-center w-full">
            <GoogleLogin
              ...
              onError={() => setErrorMessage('Google Sign-In was closed or cancelled.')}
              theme="filled_black"
              shape="pill"
              size="large"
              width="100%"
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full py-3.5 px-4 rounded-2xl bg-white hover:bg-zinc-100 text-zinc-900 font-bold text-xs shadow-xl transition flex items-center justify-center gap-3 disabled:opacity-50"
          >
            {isLoading ? (
              <>
              ...
              <>
                ...
                <span>Continue via Google OAuth Redirect</span>
              </>
            )}
          </button>

          <div className="flex items-center justify-center gap-1.5 text-[10px] text-zinc-400 font-mono pt-1">
            <Shield className="w-3 h-3 text-emerald-400" />
            <span>Official Google Client ID Active</span>
          </div>
